...
$id_notacreditocompra = $_POST['id_notacreditocompra'];
...
$notacredito = consultas::get_datos("SELECT * FROM v_notacreditocompra_det WHERE id_notacreditocompra = $id_notacreditocompra AND id_materiaprima = $id_materiaprima");
...
<div class="form-group col-xs-6">
    <label>Nuevo</label>
    <input type="number" class="form-control"  min="1" max="<?php echo $cantidad[0]['cantidad'] ?>" value="<?php echo $notacredito[0]['cantidad'] ?>" name="vcantidad" required id="cant_modificar" onKeyDown="if(this.value.length==9 && event.keyCode!=8) return false;" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="calcularmontomodificar()" onclick="calcularmontomodificar()" onkeyup="calcularmontomodificar()">
</div>
<div class="form-group col-xs-12">
